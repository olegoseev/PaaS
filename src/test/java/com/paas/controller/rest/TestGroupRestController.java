...
public class TestGroupRestController {
	...
	public void getGroup() throws Exception {
		...
		this.mockMvc.perform(get("/groups/5")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
	...
	public void getGroupNotFound() throws Exception {
		...
		this.mockMvc.perform(get("/groups/22")).andExpect(status().is4xxClientError())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
	...
	public void queryGroups() throws Exception {
		...
		this.mockMvc.perform(get("/groups/query?gid=3&name=tstsys")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
	...
	public void queryUserNotFound() throws Exception {
		...
		this.mockMvc.perform(get("/groups/query?gid=3&&name=unknown")).andExpect(status().is4xxClientError())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				.andExpect(content().json("{'status': 'error','error':'NOT_FOUND'}"));
	}
}
