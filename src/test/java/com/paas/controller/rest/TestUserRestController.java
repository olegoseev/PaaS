...
@TestPropertySource(properties = { "user.records = src/test/resources/passwd",
		"group.records = src/test/resources/group" })
...
public class TestUserRestController {
	...
	public void getUser() throws Exception {
		...
		this.mockMvc.perform(get("/users/5")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8")).andExpect(content().json(
						"{'status': 'OK','success':'OK','data':{'name':'tstgames','uid':5,'gid':60,'comment':'tstgames','home':'/usr/games'}}"));
	}

	@Test
	public void getUserNotFound() throws Exception {
		// trying to get user - games:x:5:60:games:/usr/games:/usr/sbin/nologin
		this.mockMvc.perform(get("/users/11")).andExpect(status().is4xxClientError())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
	...
	public void getAllGroupsForUser() throws Exception {
		...
		this.mockMvc.perform(get("/users/3/groups")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				// check some key : value fields in the response
				.andExpect(content().json(
						"{'status': 'OK','success':'OK', 'data':[{'name':'tstsys','gid':3},{'name':'tstadm','gid':4}]}"));
	}
	...
	public void getAllGroupsForUserNotFound() throws Exception {
		// requesting groups for user - sys:x:3:3:sys:/dev:/usr/sbin/nologin
		// the user should appear in two groups
		this.mockMvc.perform(get("/users/11/groups")).andExpect(status().is4xxClientError())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				// check some key : value fields in the response
				.andExpect(content().json("{'status': 'error','error':'NOT_FOUND'}"));
	}
	...
	public void queryUser() throws Exception {
		...
		this.mockMvc.perform(get("/users/query?uid=8&name=tstmail")).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				.andExpect(content().json("{'status': 'OK','success':'OK',"
	...
	public void queryUserNotFound() throws Exception {
		// query user - mail:x:8:8:mail:/var/mail:/usr/sbin/nologin
		this.mockMvc.perform(get("/users/query?uid=11&name=unknown")).andExpect(status().is4xxClientError())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				.andExpect(content().json("{'status': 'error','error':'NOT_FOUND'}"));
	}
}
